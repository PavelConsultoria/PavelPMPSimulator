...
export default function RelatorioExame() {
  ...
  return <div className="analisePage reportPrintPage"><header className="headerAnalise"><h1>RELATÓRIO DA SESSÃO</h1></header><main className="analiseCard">
    <section className="analiseResultado"><div><span>Resultado geral</span><strong>{percentual}%</strong></div>{sessao && <><div><span>Modo</span><strong>{sessao.modo}</strong></div><div><span>Data</span><strong>{new Date(sessao.data).toLocaleString("pt-BR")}</strong></div><div><span>Questões / respondidas</span><strong>{sessao.questoes} / {sessao.respondidas ?? sessao.questoes}</strong></div><div><span>Acertos</span><strong>{sessao.acertos}</strong></div><div><span>Duração</span><strong>{formatarDuracao(sessao.tempoSegundos)}</strong></div></>}</section>
    ...
    {sessao?.detalhes?.length ? <section className="analiseSecao reportDetails"><h2>Respostas registradas</h2><table className="reportTable"><thead><tr><th>Questão</th><th>Domínio ECO</th><th>Resposta</th><th>Resposta correta</th><th>Resultado</th></tr></thead><tbody>{sessao.detalhes.map((item, indice) => { const questao = questoesPorId.get(item.id); return <tr key={`${item.id}-${indice}`}><td>{item.id}</td><td>{questao?.dominio || "--"}</td><td>{letras(item.resposta)}</td><td>{questao ? letras(questao.corretas) : "--"}</td><td className={item.acertou ? "reportCorrect" : "reportIncorrect"}>{item.acertou ? "Correta" : "Incorreta"}</td></tr>; })}</tbody></table></section> : sessao && <p className="muted legacyReportNote">Esta sessão antiga não possui respostas por questão armazenadas. Somente os dados reais disponíveis são exibidos.</p>}
